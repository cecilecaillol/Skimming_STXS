...
int main(int argc, char** argv) {
    ...
    TTree* treePtr = (TTree*) fIn->Get("eett/final/Ntuple");
    TH1F *evCounter = (TH1F*) fIn->Get("eett/eventCount");
    TH1F *evCounterW = (TH1F*) fIn->Get("eett/summedWeights");
    HTauTauTree_eett* tree = new HTauTauTree_eett (treePtr);

    TTree *Run_Tree = new TTree("eett_tree", "eett_tree");
...
if (tree->evt==2099287305 or tree->evt==197659973) cout<<tree->evt<<" "<<tree->e1Pt<<" "<<tree->e2Pt<<" "<<tree->t1Pt<<" "<<tree->t2Pt<<endl;
        TLorentzVector dau1;
        TLorentzVector dau2;
        TLorentzVector dau3;
        TLorentzVector dau4;
...
if (tree->evt==2099287305 or tree->evt==197659973) cout<<"dz dr"<<endl;
        if ((dau1.Pt()<23.5 and dau2.Pt()<23.5) or dau1.Pt()<9.5 or dau2.Pt()<9.5 or dau3.Pt()<19.5 or dau4.Pt()<19.5) continue;
if (tree->evt==2099287305 or tree->evt==197659973) cout<<"pt"<<endl;
        if (fabs(dau1.Eta())>2.5 or fabs(dau2.Eta())>2.5 or fabs(dau3.Eta())>2.3 or fabs(dau4.Eta())>2.3) continue;
if (tree->evt==2099287305 or tree->evt==197659973) cout<<"eta"<<endl;
        bool eID1=tree->e1MVANoisoWP90;
        bool eID2=tree->e2MVANoisoWP90;
        if (!eID1 or !eID2) continue;
if (tree->evt==2099287305 or tree->evt==197659973) cout<<"eid "<<tree->e1Pt<<" "<<tree->e1Eta<<" "<<tree->e1Eta<<" "<<tree->e2Pt<<" "<<tree->e2Eta<<" "<<tree->e1PassesConversionVeto <<" "<<tree->e1MissingHits<<" "<<tree->e2PassesConversionVeto<<" "<<tree->e2MissingHits<<endl;
        if (!tree->e1PassesConversionVeto or tree->e1MissingHits>1 or !tree->e2PassesConversionVeto or tree->e2MissingHits>1) continue;
if (tree->evt==2099287305 or tree->evt==197659973) cout<<"hits conversion"<<endl;
        if (tree->t1DecayMode==5 or tree->t1DecayMode==6) continue;
        if (tree->t2DecayMode==5 or tree->t2DecayMode==6) continue;
if (tree->evt==2099287305 or tree->evt==197659973) cout<<"tau dm"<<endl;
...
if (tree->evt==2099287305 or tree->evt==197659973) cout<<"antilep"<<endl;
	evt_now=tree->evt;
	if (tree->eVetoZTTp001dxyzR0>2) continue;
	if (tree->muVetoZTTp001dxyzR0>0) continue;
if (tree->evt==2099287305 or tree->evt==197659973) cout<<tree->evt<<" "<<tree->e1Pt<<" "<<tree->e2Pt<<" "<<tree->t1Pt<<" "<<tree->t2Pt<<" "<<tree->eVetoZTTp001dxyzR0<<" "<<tree->muVetoZTTp001dxyzR0<<endl;
	if (evt_now!=evt_before){
	   lt_before=tree->e1Pt + tree->e2Pt + tree->t1Pt + tree->t2Pt;
	}
        if (evt_now!=evt_before){
           if (bestEntry>-1)
	...
	if (evt_now==evt_before){
	   if (tree->e1Pt + tree->e2Pt + tree->t1Pt + tree->t2Pt>lt_before+0.0001){
		bestEntry=iEntry;
	 	lt_before=tree->e1Pt + tree->e2Pt + tree->t1Pt + tree->t2Pt;
	   }
	}
