...
int main(int argc, char** argv) {
    ...
    TTree* treePtr = (TTree*) fIn->Get("eemt/final/Ntuple");
    TH1F *evCounter = (TH1F*) fIn->Get("eemt/eventCount");
    TH1F *evCounterW = (TH1F*) fIn->Get("eemt/summedWeights");
    HTauTauTree_eemt* tree = new HTauTauTree_eemt (treePtr);

    TTree *Run_Tree = new TTree("eemt_tree", "eemt_tree");
	...
	if (fabs(tree->e1PVDXY)>0.045) continue;
        ...
        if ((dau1.Pt()<23.5 and dau2.Pt()<23.5) or dau1.Pt()<9.5 or dau2.Pt()<9.5 or dau3.Pt()<9.5 or dau4.Pt()<9.5) continue;
        if (fabs(dau1.Eta())>2.5 or fabs(dau2.Eta())>2.5 or fabs(dau3.Eta())>2.4 or fabs(dau4.Eta())>2.3) continue;
	...
	if (evt_now!=evt_before){
	   lt_before=tree->e1Pt + tree->e2Pt + tree->mPt + tree->tPt;
	}
        if (evt_now!=evt_before){
           if (bestEntry>-1)
	...
	if (evt_now==evt_before){
	   if (tree->e1Pt + tree->e2Pt + tree->mPt + tree->tPt>lt_before+0.0001){
		bestEntry=iEntry;
	 	lt_before=tree->e1Pt + tree->e2Pt + tree->mPt + tree->tPt;
	   }
	}
