...
int main(int argc, char** argv) {
    ...
    TTree* treePtr = (TTree*) fIn->Get("ett/final/Ntuple");
    TH1F *evCounter = (TH1F*) fIn->Get("ett/eventCount");
    TH1F *evCounterW = (TH1F*) fIn->Get("ett/summedWeights");
    HTauTauTree_ett* tree = new HTauTauTree_ett (treePtr);

    TTree *Run_Tree = new TTree("ett_tree", "ett_tree");
    ...
    for (int iEntry = 0; iEntry < tree->GetEntries() ; iEntry++)
    {
        ...
        if (dau1.Pt()<24 or dau2.Pt()<19.5 or dau3.Pt()<19.5) continue;//FIXME
	...
	if (tree->eVetoZTTp001dxyzR0>1) continue;//FIXME
	if (tree->muVetoZTTp001dxyzR0>0) continue;//FIXME
	...
	if (evt_now!=evt_before){
	   lt_before=tree->ePt + tree->t1Pt + tree->t2Pt;
	}
        if (evt_now!=evt_before){
           if (bestEntry>-1)
	...
	if (evt_now==evt_before){
	   if (tree->ePt + tree->t1Pt + tree->t2Pt>lt_before+0.0001){
		bestEntry=iEntry;
	 	lt_before=tree->ePt + tree->t1Pt + tree->t2Pt;
	   }
	}
