...
int main(int argc, char** argv) {
    ...
    TTree* treePtr = (TTree*) fIn->Get("mmmt/final/Ntuple");
    TH1F *evCounter = (TH1F*) fIn->Get("mmmt/eventCount");
    TH1F *evCounterW = (TH1F*) fIn->Get("mmmt/summedWeights");
    HTauTauTree_mmmt* tree = new HTauTauTree_mmmt (treePtr);

    TTree *Run_Tree = new TTree("mmmt_tree", "mmmt_tree");
    ...
    Run_Tree->Branch("matchMu22_3", &matchMu22_3, "matchMu22_3/F");
    Run_Tree->Branch("matchTkMu22_3", &matchTkMu22_3, "matchTkMu22_3/F");
    ...
    Run_Tree->Branch("filterMu22_3", &filterMu22_3, "filterMu22_3/F");
    Run_Tree->Branch("filterTkMu22_3", &filterTkMu22_3, "filterTkMu22_3/F");
    ...
    for (int iEntry = 0; iEntry < tree->GetEntries() ; iEntry++)
    {
        ...
        dau3.SetPtEtaPhiM(tree->m3Pt,tree->m3Eta,tree->m3Phi,tree->m3Mass);
	...
	if (fabs(tree->m1PVDXY)>0.045) continue;
        ...
        if (!(tree->m1RelPFIsoDBDefault<0.20 and tree->m2RelPFIsoDBDefault<0.20) and !(tree->m1RelPFIsoDBDefault<0.20 and tree->m3RelPFIsoDBDefault<0.20) and !(tree->m2RelPFIsoDBDefault<0.20 and tree->m3RelPFIsoDBDefault<0.20)) continue;
        if ((dau1.Pt()<19.5 and dau2.Pt()<19.5 and dau3.Pt()<19.5) or dau1.Pt()<9.5 or dau2.Pt()<9.5 or dau3.Pt()<9.5 or dau4.Pt()<9.5) continue;
        if (fabs(dau1.Eta())>2.4 or fabs(dau2.Eta())>2.4 or fabs(dau3.Eta())>2.4 or fabs(dau4.Eta())>2.3) continue;
        if (tree->tDecayMode==5 or tree->tDecayMode==6) continue;
        if (!tree->tVVVLooseDeepTau2017v2p1VSjet) continue;
        if (!tree->tTightDeepTau2017v2p1VSmu) continue;
	...
	if (evt_now!=evt_before){
	   lt_before=tree->m1Pt + tree->m2Pt + tree->m3Pt + tree->tPt;
	}
        if (evt_now!=evt_before){
           if (bestEntry>-1)
	...
	if (evt_now==evt_before){
	   if (tree->m1Pt + tree->m2Pt + tree->m3Pt + tree->tPt>lt_before+0.0001){
		bestEntry=iEntry;
	 	lt_before=tree->m1Pt + tree->m2Pt + tree->m3Pt + tree->tPt;
	   }
	}
