...
int main(int argc, char** argv) {
    ...
    TTree* treePtr = (TTree*) fIn->Get("eeem/final/Ntuple");
    TH1F *evCounter = (TH1F*) fIn->Get("eeem/eventCount");
    TH1F *evCounterW = (TH1F*) fIn->Get("eeem/summedWeights");
    HTauTauTree_eeem* tree = new HTauTauTree_eeem (treePtr);

    TTree *Run_Tree = new TTree("eeem_tree", "eeem_tree");
    ...
    for (int iEntry = 0; iEntry < tree->GetEntries() ; iEntry++)
    {
        ...
        dau4.SetPtEtaPhiM(tree->mPt,tree->mEta,tree->mPhi,tree->mMass);
	...
	if (fabs(tree->e1PVDXY)>0.045) continue;
        ...
        if ((dau1.Pt()<23.5 and dau2.Pt()<23.5 and dau3.Pt()<23.5) or dau1.Pt()<9.5 or dau2.Pt()<9.5 or dau3.Pt()<9.5 or dau4.Pt()<9.5) continue;
        if (fabs(dau1.Eta())>2.5 or fabs(dau2.Eta())>2.5 or fabs(dau3.Eta())>2.5 or fabs(dau4.Eta())>2.4) continue;
	...
	if (evt_now!=evt_before){
	   lt_before=tree->e1Pt + tree->e2Pt + tree->mPt + tree->e3Pt;
	}
        if (evt_now!=evt_before){
           if (bestEntry>-1)
	...
	if (evt_now==evt_before){
	   if (tree->e1Pt + tree->e2Pt + tree->mPt + tree->e3Pt>lt_before+0.0001){
		bestEntry=iEntry;
	 	lt_before=tree->e1Pt + tree->e2Pt + tree->mPt + tree->e3Pt;
	   }
	}
