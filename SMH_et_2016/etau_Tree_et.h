...
float passEle25, passEle24Tau30, trg_singleelectron_25_eta2p1;
float matchEle25_1, matchEle24Tau30_1, matchEle24Tau30_2;
float filterEle25_1, filterEle24Tau30_1, filterEle24Tau30_2;
...
float byVVVLooseDeepVSjet_2,byVVLooseDeepVSjet_2,byVLooseDeepVSjet_2,byLooseDeepVSjet_2,byMediumDeepVSjet_2,byTightDeepVSjet_2,byVTightDeepVSjet_2,byVVTightDeepVSjet_2;
...
float pt_1_ScaleUp, pt_1_ScaleDown;
...
void fillTree(TTree *Run_Tree, HTauTauTree_et *tree, int entry_tree, int recoil, bool ismc, bool isembedded){
    ...
    TLorentzVector tau1;
    TLorentzVector tau2;
    tau1.SetPtEtaPhiM(tree->ePt,tree->eEta,tree->ePhi,0.000511);
    ...
    for (int j=0; j<27; ++j){
       ...
       else if (ismc && (tree->tZTTGenMatching==1 or tree->tZTTGenMatching==3) && tree->tDecayMode==0 && fabs(tau2.Eta())<1.5) mymetvector[j]=mymetvector[j]+tau2-1.00679*tau2;
       else if (ismc && (tree->tZTTGenMatching==1 or tree->tZTTGenMatching==3) && tree->tDecayMode==1 && fabs(tau2.Eta())<1.5) mymetvector[j]=mymetvector[j]+tau2-1.03389*tau2;
       else if (ismc && (tree->tZTTGenMatching==1 or tree->tZTTGenMatching==3) && tree->tDecayMode==0 && fabs(tau2.Eta())>1.5) mymetvector[j]=mymetvector[j]+tau2-0.965*tau2;
       else if (ismc && (tree->tZTTGenMatching==1 or tree->tZTTGenMatching==3) && tree->tDecayMode==1 && fabs(tau2.Eta())>1.5) mymetvector[j]=mymetvector[j]+tau2-1.05*tau2;
    ...
    if (ismc && (tree->tZTTGenMatching==1 or tree->tZTTGenMatching==3) && tree->tDecayMode==0 && fabs(tau2.Eta())<1.5) tau2=tau2*1.00679;
    else if (ismc && (tree->tZTTGenMatching==1 or tree->tZTTGenMatching==3) && tree->tDecayMode==1 && fabs(tau2.Eta())<1.5) tau2=tau2*1.03389;
    else if (ismc && (tree->tZTTGenMatching==1 or tree->tZTTGenMatching==3) && tree->tDecayMode==0 && fabs(tau2.Eta())>1.5) tau2=tau2*0.965;
    else if (ismc && (tree->tZTTGenMatching==1 or tree->tZTTGenMatching==3) && tree->tDecayMode==1 && fabs(tau2.Eta())>1.5) tau2=tau2*1.05;
    ...
    if (isembedded){
      if (fabs(tau1.Eta())<1.479) tau1=tau1*0.9976;
      if (fabs(tau1.Eta())>1.479) tau1=tau1*0.9930;
    }
    trg_singleelectron_25_eta2p1=tree->singleE25eta2p1TightPass && tree->eMatchesEle25Path && tree->eMatchesEle25Filter && tau1.Pt()>26;
    m_1 = 0.000511;
    e_1 = tau1.E();
    pt_1 = tau1.Pt();
    ...
    if (tree->jetVeto20>1) mjj=tree->vbfMass; //FIXME
    else mjj=-10;
