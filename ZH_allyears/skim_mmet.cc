...
int main(int argc, char** argv) {
    ...
    TTree* treePtr = (TTree*) fIn->Get("emmt/final/Ntuple");
    TH1F *evCounter = (TH1F*) fIn->Get("emmt/eventCount");
    TH1F *evCounterW = (TH1F*) fIn->Get("emmt/summedWeights");
    HTauTauTree_mmet* tree = new HTauTauTree_mmet (treePtr);

    TTree *Run_Tree = new TTree("mmet_tree", "mmet_tree");
	...
	if (tree->m1RelPFIsoDBDefault>0.2 or tree->m2RelPFIsoDBDefault>0.2) continue;
        if ((dau1.Pt()<19.5 and dau2.Pt()<19.5) or dau1.Pt()<9.5 or dau2.Pt()<9.5 or dau3.Pt()<9.5 or dau4.Pt()<19.5) continue;
        if (fabs(dau1.Eta())>2.4 or fabs(dau2.Eta())>2.4 or fabs(dau3.Eta())>2.5 or fabs(dau4.Eta())>2.3) continue;
	...
	if (evt_now!=evt_before){
	   lt_before=tree->m1Pt + tree->m2Pt + tree->ePt + tree->tPt;
	}
        if (evt_now!=evt_before){
           if (bestEntry>-1)
	...
	if (evt_now==evt_before){
	   if (tree->m1Pt + tree->m2Pt + tree->ePt + tree->tPt>lt_before+0.0001){
		bestEntry=iEntry;
	 	lt_before=tree->m1Pt + tree->m2Pt + tree->ePt + tree->tPt;
	   }
	}
